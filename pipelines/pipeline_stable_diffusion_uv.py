...
class StableDiffusionPipeline(
    ...
    def prepare_cond_image_latents(self, ccm, albedo, rm, bump, mask, cond_vae, device, num_images_per_prompt, do_classifier_free_guidance):
        dtype = self.vae.dtype

        if isinstance(ccm, list):
            ccm = torch.stack([TF.to_tensor(img) for img in ccm], dim=0).to(device=device, dtype=dtype)
        elif isinstance(ccm, torch.Tensor):
            ccm = ccm.to(device=device, dtype=dtype)

        if isinstance(albedo, list):
            albedo = torch.stack([TF.to_tensor(img) for img in albedo], dim=0).to(device=device, dtype=dtype)
        elif isinstance(albedo, torch.Tensor):
            albedo = albedo.to(device=device, dtype=dtype)

        if isinstance(rm, list):
            rm = torch.stack([TF.to_tensor(img) for img in rm], dim=0).to(device=device, dtype=dtype)
        elif isinstance(rm, torch.Tensor):
            rm = rm.to(device=device, dtype=dtype)

        if isinstance(bump, list):
            bump = torch.stack([TF.to_tensor(img) for img in bump], dim=0).to(device=device, dtype=dtype)
        elif isinstance(bump, torch.Tensor):
            bump = bump.to(device=device, dtype=dtype)

        if isinstance(mask, list):
            if isinstance(mask[0], np.ndarray):
                mask = [Image.fromarray((img*255).astype(np.uint8), mode='L') for img in mask]
                mask = [img.resize((bump.shape[3]//8, bump.shape[2]//8), resample=Image.NEAREST) for img in mask]
            elif isinstance(mask[0], Image.Image):
                mask = [img.resize((bump.shape[3]//8, bump.shape[2]//8), resample=Image.NEAREST).convert('L') for img in mask]
            ...
            mask = mask.resize((bump.shape[3]//8, bump.shape[2]//8), resample=Image.NEAREST)
        ...
        if cond_vae is not None:
            ...
            image = torch.cat([ccm, albedo, rm, bump], dim=0)
            bsz = ccm.shape[0]
            latents = self.vae.encode(image).latent_dist.mode() * self.vae.config.scaling_factor     
            if mask is not None:
                latents = torch.cat([mask, latents[0:bsz], latents[bsz:2*bsz], latents[2*bsz:3*bsz], latents[3*bsz:4*bsz]], dim=1)
            else:
                latents = torch.cat([latents[0:bsz], latents[bsz:2*bsz], latents[2*bsz:3*bsz], latents[3*bsz:4*bsz]], dim=1)
        ...
        if do_classifier_free_guidance:
            ...
            latents = torch.cat([uncond_latens, latents])
    ...
    def __call__(
        ...
        ccm_image: Optional[PipelineImageInput] = None,
        albedo_image: Optional[PipelineImageInput] = None,
        rm_image: Optional[PipelineImageInput] = None,
        bump_image: Optional[PipelineImageInput] = None,
