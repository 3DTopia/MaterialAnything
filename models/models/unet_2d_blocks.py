...
class SkipCrossAttnUpBlock2D(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        for i in range(num_layers):
            ...
            if not dual_cross_attention:
                ...
                skip_attentions.append(
                    Transformer2DModel(
                        num_attention_heads,
                        resnet_in_channels // num_attention_heads,
    ...
    def forward(
        ...
        attention_mask: Optional[torch.FloatTensor] = None,
        encoder_attention_mask: Optional[torch.FloatTensor] = None,
        use_skip_attention: Optional[bool] = False,
    ) -> torch.FloatTensor:
        if cross_attention_kwargs is not None:
            if cross_attention_kwargs.get("scale", None) is not None:
                logger.warning("Passing `scale` to `cross_attention_kwargs` is depcrecated. `scale` will be ignored.")
        ...
        for resnet, attn, skip_attn in zip(self.resnets, self.attentions, self.skip_attentions):
            # pop res hidden states
            res_hidden_states = res_hidden_states_tuple[-1]
            res_hidden_states_tuple = res_hidden_states_tuple[:-1]
            ...
            if use_skip_attention:
                res_hidden_states = res_hidden_states.reshape(res_hidden_states.shape[0], res_hidden_states.shape[1], -1).permute(0, 2, 1)
                res_hidden_states = skip_attn(
                    hidden_states,
                    encoder_hidden_states=res_hidden_states,
