...
def load_mesh(self, mesh_path, scale_factor=2.0, auto_center=True, autouv=False):
    mesh = self._load_objs_as_meshes(mesh_path, device=self.device)
    if auto_center:
...
def init_mesh_with_uv(input_path, cache_path, device):
    _, faces, aux = load_obj(input_path, device=device)
