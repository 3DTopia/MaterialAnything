...
from lib.projection_helper import (
    ...
    render_one_view_and_build_masks_materials,
    select_viewpoint,
...
def get_3d_iou(box1, box2):
    ...
    inter_size = max(0, inter_box[3] - inter_box[0]) * max(0, inter_box[4] - inter_box[1]) * max(0, inter_box[5] - inter_box[2])
    box1_size = (box1[3] - box1[0]) * (box1[4] - box1[1]) * (box1[5] - box1[2])
    box2_size = (box2[3] - box2[0]) * (box2[4] - box2[1]) * (box2[5] - box2[2])
    iou = inter_size / (box1_size + box2_size - inter_size)
    return iou
...
if __name__ == "__main__":
    ...
    # init resources
    # init mesh
    mesh, _, faces, aux, principle_directions, mesh_center, mesh_scale = init_mesh_with_uv(
    ...
    init_mask = Image.new('RGB', (1024, 1024), color=(0, 0, 0))
    ...
    exist_albedo = torch.from_numpy(np.zeros([args.uv_size, args.uv_size]).astype(np.float32)).to(DEVICE)
    exist_roughness_metallic = torch.from_numpy(np.zeros([args.uv_size, args.uv_size]).astype(np.float32)).to(DEVICE)
    exist_bump = torch.from_numpy(np.zeros([args.uv_size, args.uv_size]).astype(np.float32)).to(DEVICE)
    exist_mask = torch.from_numpy(np.zeros([args.uv_size, args.uv_size]).astype(np.float32)).to(DEVICE)
    ...
    materialSD = get_image2materials(args.image2materials_model, DEVICE)
    uvrefiner = get_uvrefiner(args.uvrefine_model, DEVICE)
    ...
    material_dir = os.path.join(generate_dir, "material")
    ...
    def uv_to_3d(mesh, texture_dims) -> torch.Tensor:
        # Get the UV coordinates and faces from the mesh
        textures = mesh.textures
        if not isinstance(textures, TexturesUV):
            raise ValueError("Mesh must have UV textures.")
            
        verts_uvs = textures.verts_uvs_padded()[0]  # (V, 2) UV coordinates for each vertex
        faces_uvs = textures.faces_uvs_padded()[0]  # (F, 3) index into verts_uvs
        verts = mesh.verts_padded()[0]  # (V, 3) 3D vertices
        faces = mesh.faces_padded()[0]  # (F, 3) index into verts

        uv_face_attr = verts_uvs[faces_uvs].unsqueeze(0)    # (1, F, 3, 2)
        ...
        face_vertices_z = torch.zeros_like(face_vertices_world[:, :, :, -1], device='cuda')
        uv_position, face_idx = kal.render.mesh.rasterize(texture_dims[0], texture_dims[1], face_vertices_z, uv_face_attr * 2 - 1, face_features=face_vertices_world)
        uv_position = torch.clamp(uv_position, -1, 1)

        uv_position  = uv_position * 2 # [-0.5, 0.5] to [-1, 1]
    ...
    import cv2
    # Example usage:
    uv_position = uv_to_3d(mesh, (args.uv_size, args.uv_size))
    # save uv_position as an image
    uv_position = Image.fromarray((uv_position[0].cpu().numpy() * 255).astype(np.uint8))

    uv_array = np.array(uv_position)
    ...
    dilated_uv = np.copy(uv_array)
    
    uv_array[np.all(uv_array == [255, 255, 255], axis=-1)] = 0
    dilated_uv = cv2.dilate(uv_array, kernel, iterations=5)
    uv_array = np.array(uv_position)
    dilated_uv_final = np.where(dilated_mask == 255, dilated_uv, uv_array)
    uv_position = Image.fromarray(dilated_uv_final.astype(np.uint8))

    uv_position.save(os.path.join(material_dir, "uv_ccm_dilated.png"))
    # # uv_position get not (0,0,0) mask
    uv_array = np.array(uv_position)
    uv_mask = np.any(uv_array != [255, 255, 255], axis=-1)
    uv_mask = Image.fromarray(uv_mask.astype(np.uint8) * 255)
    uv_mask.save(os.path.join(material_dir, "uv_mask.png"))
    ...
    for view_idx in range(NUM_PRINCIPLE):
        ...
        (
            ...
            keep_mask_tensor, update_mask_tensor, generate_mask_tensor, all_mask_tensor, quad_mask_tensor, valid_mask_tensor
        ) = render_one_view_and_build_masks_materials(dist, elev, azim, 
            view_idx, view_idx, view_punishments, # => actual view idx and the sequence idx 
            pre_similarity_texture_cache, 
            exist_texture, init_albedo, init_roughness_metallic, init_bump,
        ...
        _, _, exist_texture = backproject_from_image(
        ...
        init_materials_mask = 1 - generate_mask_tensor
        albedo_image, rm_image, bump_image = apply_material_estimation(materialSD, args.prompt, generate_image, normal_map, init_materials, init_materials_mask, 768, 768, DEVICE)
        albedo_image.save(os.path.join(inpainted_image_dir, "{}_albedo.png".format(view_idx)))
        rm_image.save(os.path.join(inpainted_image_dir, "{}_rm.png".format(view_idx)))
        _, r_image, m_image = rm_image.split()
        r_image.save(os.path.join(inpainted_image_dir, "{}_roughness.png".format(view_idx)))
        m_image.save(os.path.join(inpainted_image_dir, "{}_metallic.png".format(view_idx)))
        bump_image.save(os.path.join(inpainted_image_dir, "{}_bump.png".format(view_idx)))

        # project albedo
        albedo_views.append(albedo_image)
        init_albedo, _, exist_albedo = backproject_from_image(
            mesh, faces, new_verts_uvs, cameras, 
            albedo_image, generate_mask_image, generate_mask_image, init_albedo, exist_albedo, 
            args.image_size * args.render_simple_factor, args.uv_size, args.fragment_k,
            DEVICE, blending_weights=0.0
        )

        # project roughness matallic
        rm_views.append(rm_image)
        init_roughness_metallic, _, exist_roughness_metallic = backproject_from_image(
            mesh, faces, new_verts_uvs, cameras, 
            rm_image, generate_mask_image, generate_mask_image, init_roughness_metallic, exist_roughness_metallic, 
            args.image_size * args.render_simple_factor, args.uv_size, args.fragment_k,
            DEVICE, blending_weights=0.0
        )

        # project bump
        bump_views.append(bump_image)
        init_bump, _, exist_bump = backproject_from_image(
            mesh, faces, new_verts_uvs, cameras, 
            bump_image, generate_mask_image, generate_mask_image, init_bump, exist_bump, 
            args.image_size * args.render_simple_factor, args.uv_size, args.fragment_k,
            DEVICE, blending_weights=0.0
        )
        ...
        init_mask, _, exist_mask = backproject_from_image(
            ...
            args.image_size * args.render_simple_factor, args.uv_size, args.fragment_k,
            DEVICE, blending_weights=1.0
        )
        # dilate init_mask
        # init_mask = init_mask.filter(ImageFilter.MaxFilter(5))
        # # binarize init_mask
        # init_mask = init_mask.point(lambda p: p > 0 and 255)
        init_mask.save(os.path.join(material_dir, "{}_mask.png".format(view_idx)))

        # get the edge of init_mask
        init_mask_edge = init_mask.filter(ImageFilter.FIND_EDGES)
        for _ in range(4):
            init_mask_edge = init_mask_edge.filter(ImageFilter.MaxFilter(5))
        init_mask_edge = Image.composite(init_mask_edge, Image.new('RGB', (1024, 1024), color=(0, 0, 0)), uv_mask)
        init_mask_edge.save(os.path.join(material_dir, "{}_mask_edge.png".format(view_idx)))

        all_mask_image = Image.fromarray((np.array(all_mask_tensor[0].cpu().numpy()) * 255).astype(np.uint8))
        init_all_mask = Image.new('RGB', (1024, 1024), color=(0, 0, 0))
        init_all_mask, _, _ = backproject_from_image(
            mesh, faces, new_verts_uvs, cameras, 
            all_mask_image.convert('RGB'), all_mask_image, all_mask_image, init_all_mask, torch.zeros_like(exist_mask), 
            args.image_size * args.render_simple_factor, args.uv_size, args.fragment_k,
            DEVICE, blending_weights=1.0
        )
        mask_views.append(init_all_mask)


        # set background to black using uv_mask
        # add blur for init_albedo at mask edge
        init_albedo_blurred = init_albedo.filter(ImageFilter.GaussianBlur(15))
        # use init_mask_edge to blend init_albedo_blurred and init_albedo
        init_albedo = Image.composite(init_albedo_blurred, init_albedo, init_mask_edge.convert('L'))
        init_albedo.save(os.path.join(material_dir, "{}_albedo.png".format(view_idx)))


        albedo_image, rm_image, bump_image = apply_uv_refinement(uvrefiner, init_albedo, init_roughness_metallic, init_bump, uv_position, init_mask_edge, 1024, 1024, DEVICE)

        albedo_image.save(os.path.join(material_dir, "{}_albedo_after.png".format(view_idx)))
        rm_image.save(os.path.join(material_dir, "{}_rm_after.png".format(view_idx)))
        bump_image.save(os.path.join(material_dir, "{}_bump_after.png".format(view_idx)))
        ...
        (
            view_score,
            renderer, cameras, fragments,
            init_image, init_materials, *_,
        ) = render_one_view_and_build_masks_materials(dist, elev, azim, 
            view_idx, view_idx, view_punishments, # => actual view idx and the sequence idx 
            pre_similarity_texture_cache, exist_texture,
            init_albedo, init_roughness_metallic, init_bump,
            mesh, faces, new_verts_uvs,
            args.image_size, args.fragment_k,
            init_image_dir, mask_image_dir, normal_map_dir, depth_map_dir, similarity_map_dir,
        ...
        if not args.no_update and update_mask_tensor.sum() > 0 and update_mask_tensor.sum() / (all_mask_tensor.sum()) > 0.05:
            ...
            # set the background of generate_image to white using all_mask_tensor
            diffused_image_after = np.array(diffused_image_after)
            diffused_image_after[valid_mask_tensor.cpu().numpy() == 0] = 255
            diffused_image_after = Image.fromarray(diffused_image_after)
            diffused_image_after.save(os.path.join(inpainted_image_dir, "{}_update_wb.png".format(view_idx)))

            init_materials_mask = 1 - generate_mask_tensor
            init_materials_mask[all_mask_tensor==0] = 1
            albedo_image, rm_image, bump_image = apply_material_estimation(materialSD, args.prompt, diffused_image_after, normal_map, init_materials, init_materials_mask, 768, 768, DEVICE)
            ...
            init_albedo, _, exist_albedo = backproject_from_image(
                mesh, faces, new_verts_uvs, cameras, 
                albedo_image, update_mask_image, update_mask_image, init_albedo, exist_albedo, 
                args.image_size * args.render_simple_factor, args.uv_size, args.fragment_k,
                DEVICE, blending_weights=0.5
            )
            ...
            init_roughness_metallic, _, exist_roughness_metallic = backproject_from_image(
                mesh, faces, new_verts_uvs, cameras, 
                rm_image, update_mask_image, update_mask_image, init_roughness_metallic, exist_roughness_metallic, 
                args.image_size * args.render_simple_factor, args.uv_size, args.fragment_k,
                DEVICE
            )
            ...
            init_bump, _, exist_bump = backproject_from_image(
                mesh, faces, new_verts_uvs, cameras, 
                bump_image, update_mask_image, update_mask_image, init_bump, exist_bump, 
                args.image_size * args.render_simple_factor, args.uv_size, args.fragment_k,
                DEVICE
            )
            ...
            # albedo_image.save(os.path.join(material_dir, "{}_albedo_after.png".format(view_idx)))   
            # rm_image.save(os.path.join(material_dir, "{}_rm_after.png".format(view_idx)))
            # rm_image = rm_image.convert("RGB")
            # _, roughness_image, metallic_image = rm_image.split()
            # roughness_image.save(os.path.join(material_dir, "{}_roughness_after.png".format(view_idx)))
            # metallic_image.save(os.path.join(material_dir, "{}_metallic_after.png".format(view_idx)))
            # bump_image.save(os.path.join(material_dir, "{}_bump_after.png".format(view_idx)))
        ...
        # save init_texture, init_roughness_mateliic and init_bump
        init_texture.save(os.path.join(material_dir, "{}_texture.png".format(view_idx)))
        init_albedo.save(os.path.join(material_dir, "{}_albedo.png".format(view_idx)))
        # split init_roughness_metallic into G channel (init_roughness) and B channel(init_metallic)
        init_roughness_metallic = init_roughness_metallic.convert("RGB")
        _, init_roughness, init_metallic = init_roughness_metallic.split()
        init_roughness.save(os.path.join(material_dir, "{}_roughness.png".format(view_idx)))
        init_metallic.save(os.path.join(material_dir, "{}_metallic.png".format(view_idx)))
        init_roughness_metallic.save(os.path.join(material_dir, "{}_rm.png".format(view_idx)))
        init_bump.save(os.path.join(material_dir, "{}_bump.png".format(view_idx)))
    ...
    # masking pre_similarity_texture_cache using mask_views. pre_similarity_texture_cache is tensor
    for i in range(len(pre_similarity_texture_cache)):
        mask = mask_views[i].convert('L')
        mask = (transforms.ToTensor()(mask).to(DEVICE) > 0).float()
        pre_similarity_texture_cache[i] = pre_similarity_texture_cache[i] * mask
    
    final_albedo = bake_texture(
        albedo_views, 
        mesh, 
        pre_dist_list, 
        pre_elev_list, 
    ...
    final_bump = bake_texture(bump_views, mesh, pre_dist_list, pre_elev_list, pre_azim_list, pre_similarity_texture_cache, image_size = args.image_size, uv_size=args.uv_size, exp=6, device=DEVICE)

    albedo_image, rm_image, bump_image = apply_uv_refinement(uvrefiner, final_albedo, final_rm, final_bump, uv_position, init_mask, 1024, 1024, DEVICE)

    from lib.voronoi import voronoi_solve

    # convert to tensor
    # albedo_image_tensor - A multi-channel tensor, (H, W, C)
    # uv_mask_tensor - A single-channel tensor, (H, W)
    uv_mask_tensor = transforms.ToTensor()(uv_mask).to(DEVICE)
    albedo_image_tensor = transforms.ToTensor()(albedo_image).to(DEVICE)
    rm_image_tensor = transforms.ToTensor()(rm_image).to(DEVICE)
    bump_image_tensor = transforms.ToTensor()(bump_image).to(DEVICE)
    with torch.no_grad():
        albedo_image_tensor = voronoi_solve(albedo_image_tensor.permute(1,2,0), uv_mask_tensor[0], DEVICE)
        rm_image_tensor = voronoi_solve(rm_image_tensor.permute(1,2,0), uv_mask_tensor[0], DEVICE)
        bump_image_tensor = voronoi_solve(bump_image_tensor.permute(1,2,0), uv_mask_tensor[0], DEVICE)

    # import pdb; pdb.set_trace()
    albedo_image = Image.fromarray((albedo_image_tensor.cpu().numpy() * 255).astype(np.uint8))
    rm_image = Image.fromarray((rm_image_tensor.cpu().numpy() * 255).astype(np.uint8))
    bump_image = Image.fromarray((bump_image_tensor.cpu().numpy() * 255).astype(np.uint8))

    final_albedo.save(os.path.join(material_dir, "final_albedo.png"))
    final_rm.save(os.path.join(material_dir, "final_rm.png"))
    final_rm = final_rm.convert("RGB")
    _, final_roughness, final_metallic = final_rm.split()
    final_roughness.save(os.path.join(material_dir, "final_roughness.png"))
    final_metallic.save(os.path.join(material_dir, "final_metallic.png"))
    final_bump.save(os.path.join(material_dir, "final_bump.png"))

    init_texture.save(os.path.join(material_dir, "final_albedo_after_src.png")) # save original texture
    albedo_image.save(os.path.join(material_dir, "final_albedo_after.png"))   
    rm_image.save(os.path.join(material_dir, "final_rm_after.png"))
    rm_image = rm_image.convert("RGB")
    _, roughness_image, metallic_image = rm_image.split()
    roughness_image.save(os.path.join(material_dir, "final_roughness_after.png"))
    metallic_image.save(os.path.join(material_dir, "final_metallic_after.png"))
    bump_image.save(os.path.join(material_dir, "final_bump_after.png"))


    # 2. update texture with RePaint 
    #################### NOTE this section is not used for the paper #################
    ...
    if args.update_steps > 0:
        ...
        for view_idx in range(args.update_steps):
            ...
            _, project_mask_image, exist_texture = backproject_from_image(
                mesh, faces, new_verts_uvs, cameras, 
