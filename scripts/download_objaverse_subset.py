...
SUBSET = "./data/objaverse_complex.txt"
DATA_DIR = "./data/objaverse_complex"
...
def collapse_objects(input_path, output_path, device=DEVICE, inplace=False):
    # Load the mesh with vertices, faces, and auxiliary data
    verts, faces, aux = load_obj(input_path, device=device)
    
    # Extract vertex normals from the auxiliary data
    # verts_normals = aux.normals if aux.normals is not None else None
    # faces_normals_idx = faces.normals_idx if faces.normals_idx is not None else None
    ...
    # Save the OBJ with vertex normals included
    save_obj(
        str(output_path),
        verts=verts,
